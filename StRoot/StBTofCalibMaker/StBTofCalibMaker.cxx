...
Int_t StBTofCalibMaker::InitRun(int runnumber)
{
    ...
    if(vpdCalib) {
        ...
        // Check to see if this is a simulated run
        fBFChain = dynamic_cast<StBFChain*>(StMaker::GetChain());
        if (fBFChain) {
            if (fBFChain->GetOption("vpdSim")) {
                mUseVpdStart = kTRUE;
            }
        }
...
void StBTofCalibMaker::processMuDst()
{
    ...
    for(int i=0;i<nhits;i++) {
        ...
        isMcFlag = kFALSE;       // Check to see if the hit is simulated.
        LOG_INFO << "The qaTruth is: " << aHit->qaTruth();
        if ( aHit->qaTruth() == 1  ) {
            isMcFlag = kTRUE;
            LOG_DEBUG << "Simulated hit." << endm;
        }
...
void StBTofCalibMaker::loadVpdData()
{
    ...
    // Check to see if this is a simulated run
    if (fBFChain) {
        if (fBFChain->GetOption("Simu")) {
            isMcFlag = kTRUE;
            LOG_INFO << "Simulated Vpd (found 'Simu' chain option)" << endm;
        }
        else {
            LOG_INFO << "Real Data Vpd ('Simu' chain option not present)" << endm;
        }
    }
    
    for(int i=0;i<mNVPD;i++) {
        mVPDLeTime[i] = mBTofHeader->vpdTime(west, i+1);
        if (isMcFlag) {     // handles simulation for west
    ...
    for(int i=0;i<mNVPD;i++) {
        ...
        if (isMcFlag) {     // handles simulation for east
...
void StBTofCalibMaker::tstart(const Double_t vz, Double_t *tstart, Double_t *tdiff)
{
    ...
    *tdiff = -9999.;
    ...
    LOG_INFO << "mNEast = " << mNEast << ", mNWest = " << mNWest << endm;
    LOG_INFO << "mVPDEastHitsCut = " << mVPDEastHitsCut << ", mVPDWestHitsCut = " << mVPDWestHitsCut << endm;
    ...
    *tstart = -9999.;
    ...
    for(int i=0;i<nBTofHits;i++) {
        ...
        if(trayId>0&&trayId<=mNTray) {
            ...
            Double_t tofcorr = tof;
            LOG_INFO << "The tof is: " << tofcorr << endm;
            if (!isMcFlag) {
                tofcorr = tofAllCorr(tof, tot, zhit, trayId, moduleChan);
                if(tofcorr<0.) {
                    LOG_WARN << " Calibration failed! ... " << endm;
