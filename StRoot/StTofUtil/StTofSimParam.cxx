...
#include "tables/St_tofSimPars_Table.h"
...
void StTofSimParam::init(const int date, const int time, const char* Default_time){
  ...
  // mSimParam = *sim;
    ...
    dbMk->SetDebug();
    dbMk->SetDateTime(20040131,0); // event or run start time, set to your liking
    dbMk->SetFlavor("ofl");
    ...
    DB = dbMk->GetInputDB("Calibrations/tof/tofSimPars");
    if (!DB) {
        LOG_WARN << "Failed to connect to Database!" << endm;
    }

    //    const char* time = "2004-01-31 09:51:58";   // Temporarily hard-coded
    
    St_tofSimPars *dataset = 0;
    dataset = (St_tofSimPars*) DB->Find("tofSimPars");
    Int_t rows = dataset->GetNRows();
    if (rows > 1) {
        LOG_INFO << "INFO: found INDEXED table with " << rows << " rows" << endm;
    }
    
    if (dataset) {
        TDatime val[3];
        dbMk->GetValidity((TTable*)dataset,val);
        tofSimPars_st *table = dataset->GetTable();
    
        // Store the database values into the member variables (overwrite the default values)
        m_d_gap = table->d_gap;
        m_nmax_clus = table->nClustersMax;
        m_vd_mean = table->vDriftMean;
        m_nmean_e = table->nEperCluster;
        m_nmax_e = table->nEMax;
        m_adc_thre = table->Adcthreshold;
        m_res_tof = table->timeRes;
        m_er = table->er;
        m_toffset = table->T0;
    }
    else {
